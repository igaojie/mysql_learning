...
2. 堆表的缺点：
 	1. 对于主键的范围查询效率不高，不过对于主键的查询不常用。
 	2. DML操作的时候，索引维护量比较大，如果一个记录的位置发生变化，那么所有的索引指针都要发生变化。而索引组织表却不是。只需要修改一个主键索引指针的位置就可以了。
 	3. 
...
### 查看哪些表没有主键？information\_schema.STATISTICS 
...
(root@localhost)[information_schema]> SELECT
    ...
    ->         STATISTICS
    ->     WHERE
    ->         INDEX_NAME = 'PRIMARY') AS b ON a.table_name = b.table_name
    -> WHERE
    ->     1 = 1 AND b.TABLE_NAME IS NULL;
+------------+
| table_name |
+------------+
| t          |
...
(root@localhost)[dbt3]> select *  from information_schema.tables t  left join  information_schema.statistics s  on t.table_schema = s.table_schema and t.table_name  = s.table_name and s.index_name  = 'PRIMARY' where t.table_schema not in ('mysql','performance_schema','information_schema','sys') and table_type = 'BASE TABLE' and s.index_name is null;
+---------------+--------------+-----------------+------------+--------+---------+------------+------------+----------------+-------------+-----------------+--------------+-----------+----------------+---------------------+-------------+------------+--------------------+----------+----------------+---------------+---------------+--------------+------------+------------+--------------+------------+--------------+-------------+-----------+-------------+----------+--------+----------+------------+---------+---------------+
| TABLE_CATALOG | TABLE_SCHEMA | TABLE_NAME      | TABLE_TYPE | ENGINE | VERSION | ROW_FORMAT | TABLE_ROWS | AVG_ROW_LENGTH | DATA_LENGTH | MAX_DATA_LENGTH | INDEX_LENGTH | DATA_FREE | AUTO_INCREMENT | CREATE_TIME         | UPDATE_TIME | CHECK_TIME | TABLE_COLLATION    | CHECKSUM | CREATE_OPTIONS | TABLE_COMMENT | TABLE_CATALOG | TABLE_SCHEMA | TABLE_NAME | NON_UNIQUE | INDEX_SCHEMA | INDEX_NAME | SEQ_IN_INDEX | COLUMN_NAME | COLLATION | CARDINALITY | SUB_PART | PACKED | NULLABLE | INDEX_TYPE | COMMENT | INDEX_COMMENT |
+---------------+--------------+-----------------+------------+--------+---------+------------+------------+----------------+-------------+-----------------+--------------+-----------+----------------+---------------------+-------------+------------+--------------------+----------+----------------+---------------+---------------+--------------+------------+------------+--------------+------------+--------------+-------------+-----------+-------------+----------+--------+----------+------------+---------+---------------+
| def           | dbt3         | time_statistics | BASE TABLE | InnoDB |      10 | Dynamic    |         15 |           1092 |       16384 |               0 |            0 |         0 |           NULL | 2018-01-03 22:40:49 | NULL        | NULL       | latin1_swedish_ci  |     NULL |                |               | NULL          | NULL         | NULL       |       NULL | NULL         | NULL       |         NULL | NULL        | NULL      |        NULL |     NULL | NULL   | NULL     | NULL       | NULL    | NULL          |
| def           | test         | a               | BASE TABLE | InnoDB |      10 | Dynamic    |         13 |           1260 |       16384 |               0 |            0 |         0 |           NULL | 2018-01-02 19:43:02 | NULL        | NULL       | utf8_general_ci    |     NULL |                |               | NULL          | NULL         | NULL       |       NULL | NULL         | NULL       |         NULL | NULL        | NULL      |        NULL |     NULL | NULL   | NULL     | NULL       | NULL    | NULL          |
| def           | test         | t               | BASE TABLE | InnoDB |      10 | Dynamic    |          0 |              0 |       16384 |               0 |        16384 |         0 |           NULL | 2018-01-06 00:10:20 | NULL        | NULL       | utf8mb4_general_ci |     NULL |                |               | NULL          | NULL         | NULL       |       NULL | NULL         | NULL       |         NULL | NULL        | NULL      |        NULL |     NULL | NULL   | NULL     | NULL       | NULL    | NULL          |
| def           | test         | t1              | BASE TABLE | InnoDB |      10 | Dynamic    |         13 |           1260 |       16384 |               0 |        16384 |         0 |           NULL | 2018-01-07 17:52:23 | NULL        | NULL       | utf8_general_ci    |     NULL |                |               | NULL          | NULL         | NULL       |       NULL | NULL         | NULL       |         NULL | NULL        | NULL      |        NULL |     NULL | NULL   | NULL     | NULL       | NULL    | NULL          |
+---------------+--------------+-----------------+------------+--------+---------+------------+------------+----------------+-------------+-----------------+--------------+-----------+----------------+---------------------+-------------+------------+--------------------+----------+----------------+---------------+---------------+--------------+------------+------------+--------------+------------+--------------+-------------+-----------+-------------+----------+--------+----------+------------+---------+---------------+
4 rows in set (0.10 sec)

```
### 找出哪些索引创建了却从来没有使用过的？sys.schema\_unused_indexes

```
(root@localhost)[information_schema]> use sys;
Reading table information for completion of table and column names
You can turn off this feature to get a quicker startup with -A
...
| employees     | customer     | i_c_nationkey         |
| employees     | departments  | dept_name             |
| employees     | dept_emp     | dept_no               |
| employees     | dept_manager | dept_no               |
| employees     | lineitem     | i_l_shipdate          |
| employees     | lineitem     | i_l_suppkey_partkey   |
| employees     | lineitem     | i_l_partkey           |
| employees     | lineitem     | i_l_suppkey           |
| employees     | lineitem     | i_l_receiptdate       |
| employees     | lineitem     | i_l_orderkey          |
| employees     | lineitem     | i_l_orderkey_quantity |
| employees     | lineitem     | i_l_commitdate        |
| test          | t            | idx_b                 |
+---------------+--------------+-----------------------+
...
```
(root@localhost)[sys]> use dbt3;
Reading table information for completion of table and column names
You can turn off this feature to get a quicker startup with -A

Database changed
(root@localhost)[dbt3]> select * from orders orders limit 1\G
*************************** 1. row ***************************
     o_orderkey: 1
      o_custkey: 36901
  o_orderstatus: O
...
+------------+-----------+---------------+--------------+-------------+-----------------+-----------------+----------------+--------------------------------------------------------------------------------+
|    1750466 |     21433 | F             |    555285.16 | 1992-11-30  | 4-NOT SPECIFIED | Clerk#000000040 |              0 | furiously even packages on the carefully bold instructions are blithely along  |
|    4722021 |    128120 | F             |    544089.09 | 1994-04-07  | 1-URGENT        | Clerk#000000230 |              0 | furiously unusual theodolite                                                   |
|    3043270 |    144617 | O             |    530604.44 | 1997-02-12  | 5-LOW           | Clerk#000000699 |              0 | blithely unusual ideas are quickly after the instructi                         |
|    4576548 |    108931 | O             |    525590.57 | 1997-12-26  | 1-URGENT        | Clerk#000000336 |              0 | instructions according to the pa                                               |
|    2232932 |     13940 | O             |    522720.61 | 1997-04-13  | 2-HIGH          | Clerk#000000245 |              0 | carefully express accounts alongside of the slyly even asymptotes wake quick   |
|    3586919 |     24049 | F             |    522644.48 | 1992-11-07  | 1-URGENT        | Clerk#000000924 |              0 | carefully final ideas nag regular accounts. even ideas serve never quickly reg |
|    2199712 |     66790 | O             |    515531.82 | 1996-09-30  | 2-HIGH          | Clerk#000000650 |              0 | ironic asymptotes in p                                                         |
|    2185667 |     51796 | F             |    511359.88 | 1992-10-08  | 1-URGENT        | Clerk#000000574 |              0 | quickly even theodolites wake against the accounts. slyly dogged deposi        |
|    4515876 |    100685 | F             |     510061.6 | 1993-11-02  | 4-NOT SPECIFIED | Clerk#000000105 |              0 | even deposits play blithely regular frets. carefully special theodolit         |
|     972901 |    141100 | F             |    508668.52 | 1992-07-18  | 3-MEDIUM        | Clerk#000000667 |              0 | final, bold pinto beans cajole frets.                                          |
+------------+-----------+---------------+--------------+-------------+-----------------+-----------------+----------------+--------------------------------------------------------------------------------+
10 rows in set (5.46 sec)

//以上sql语句执行了5.46s ，通过慢查询日志可以观察到确实是超过了设置的慢查询时间
tail -f slow.log
# Time: 2018-01-07T06:33:45.531039Z
# User@Host: root[root] @ localhost []  Id:    15
# Query_time: 5.456762  Lock_time: 0.001241 Rows_sent: 10  Rows_examined: 1500010
SET timestamp=1515306825;
...
+----+-------------+--------+------------+------+---------------+------+---------+------+---------+----------+----------------+
|  1 | SIMPLE      | orders | NULL       | ALL  | NULL          | NULL | NULL    | NULL | 1486955 |   100.00 | Using filesort |
+----+-------------+--------+------------+------+---------------+------+---------+------+---------+----------+----------------+
